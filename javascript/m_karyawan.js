...
// console.log('cek login',store.user.data)
const swal = inject('swal')
const is_superadmin = (store.user.data.is_superadmin)
...
watch(
  () => [valuesKontrak.tgl_awal, valuesKontrak.duration],
  () => {
    hitungTanggalAkhir()
  }
)
...
async function hitungTanggalAkhir() {
  console.log('ini di pencet coi')

  const { tgl_awal, duration } = valuesKontrak
  ...
  try {
    ...
    valuesKontrak.tgl_akhir = data?.tgl_akhir || ''
  } catch (err) {
...
onBeforeMount(async () => {
  if (isRead) {
    ...
    try {
      ...
      const params = { transform: true, detail: true }
      ...
      initialValues.m_kary_det_kontrak?.reverse().forEach((dt) => {
        detailKont.value = [dt, ...detailKont.value]
      })
      ...
      if (initialValues.info_cuti) {
        ...
        values.cuti_1_hari = initialValues.info_cuti.cuti_satu_hari;
        values.sisa_cuti_1_hari = initialValues.info_cuti.sisa_cuti_satu_hari;
        values.cuti_setengah_hari = initialValues.info_cuti.cuti_setengah_hari;
        values.sisa_cuti_setengah_hari = initialValues.info_cuti.sisa_cuti_setengah_hari;
        values.cuti_setengah_terpakai = initialValues.info_cuti.cuti_terpakai;
        values.masa_kerja = initialValues.info_cuti.masa_kerja;
        values.remaining_days = initialValues.info_cuti.remaining_days;
      }
      ...
      if (initialValues['m_kary_det_kontrak']) {
        detailKont.value = []
        for (const item of initialValues['m_kary_det_kontrak']) {
          try {
            ...
            if (!resTipe.ok) throw new Error("Failed when trying to read tipe_karyawan")
            const resultTipe = await resTipe.json()
            const tipeData = resultTipe.data

            const resDir = await fetch(`${store.server.url_backend}/operation/m_dir/${item.m_dir_id}`, {
              headers: {
                'Content-Type': 'Application/json',
                Authorization: `${store.user.token_type} ${store.user.token}`
              },
            })
            if (!resDir.ok) throw new Error("Failed when trying to read m_dir")
            const resultDir = await resDir.json()
            const dirData = resultDir.data

            const resDivisi = await fetch(`${store.server.url_backend}/operation/m_divisi/${item.m_divisi_id}`, {
              headers: {
                'Content-Type': 'Application/json',
                Authorization: `${store.user.token_type} ${store.user.token}`
              },
            })
            if (!resDivisi.ok) throw new Error("Failed when trying to read m_divisi")
            const resultDivisi = await resDivisi.json()
            const divisiData = resultDivisi.data

            detailKont.value.push({
              ...item,
              tipeKary: tipeData?.value || '-',
              m_dir_nama: dirData?.nama || '-',
              nameDiv: divisiData?.nama || '-',
              _id: ++_idKont
            })
          } catch (err) {
...
//   for (const key in valuesKontrak) {
//     if (key !== 'desc') {
//       if (valuesKontrak[key] == null) {
//         tempObj[key] = ['Bidang ini wajib diisi']
//       }
//     }
//   }
...
//     return
//   }
//   detailKont.value = [...detailKont.value, { ...valuesKontrak }]
//   Object.keys(valuesKontrak).forEach(key => valuesKontrak[key] = null)
...
//   detailKont.value = [...detailKont.value, { ...valuesKontrak }]
//   if (valuesKontrak.status === true || valuesKontrak.status === 1) {
//     values.kontrak = [...(values.kontrak || []), valuesKontrak.tgl_akhir]
//   }
//   Object.keys(valuesKontrak).forEach(key => valuesKontrak[key] = null)
//   formErrorsKont.value = {}
// }
...
const addKontrak = async () => {
  ...
  const newKontrak = { ...valuesKontrak, status: true }

  detailKont.value = [...detailKont.value, newKontrak]

  values.m_dir_id = newKontrak.m_dir_id
  values.tipe_karyawan_id = newKontrak.tipe_karyawan_id
  values.status = newKontrak.status
  values.kontrak = [newKontrak.tgl_akhir]
...
const removeDetail = (index) => {
  detailKont.value.splice(index, 1)
}
...
function uploadFile(file) {
  ...
  const dataURL = `${store.server.url_backend}/public/m_kary/importKary`
...
const downloadExcel = async () => {
  try {
    const response = await fetch(`${store.server.url_backend}/public/m_kary/exportKary`)
  ...
  try {
    ...
    if (resultJson.data.length > 0) {
      resultJson.data.forEach((item) => {
        if (!optGroup.includes(item.tipe_karyawan_value)) {
          optGroup.push(item.tipe_karyawan_value)
        }
      })
      console.log('optGroup unique:', optGroup)
    }
  } catch (err) {
    isBadForm.value = true
    swal.fire({
...
//         optGroup.push(item.tipe_karyawan_value)
//       })
//       optGroup = [...new Set(optGroup)]  // Ini supaya jadi unik
//       console.log('optGroup unique:', optGroup)
//     }
//   } catch (err) {
//     isBadForm.value = true
...
const landing = reactive({
      ...
      click(row) {
        swal.fire({
          ...
          if (result.isConfirmed) {
            try {
              const dataURL = `${store.server.url_backend}/operation${endpointApi}/delete_kary?id=${row.id}`
              isRequesting.value = true
              const res = await fetch(dataURL, {
                method: 'POST',
                headers: {
  ...
  api: {
    ...
    params: {
      ...
      transform: false,
      scopes: 'landing',
      // selectfield: 'id, kode, nama_depan, tipe_karyawan.value as "tipe_karyawan.value"',
      searchfield: 'this.id, this.nik, this.kode, atasan.nama_lengkap, this.nama_lengkap, m_divisi.nama, this.alamat_domisili, this.no_tlp',
    },

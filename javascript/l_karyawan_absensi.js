...
const onGenerate = async () => {
  ...
  if (values.is_active !== undefined && values.is_active !== null) {
    tempGet.push(`is_active=${values.is_active}`)
  }
  ...
  if (values.tipe?.toLowerCase() == 'pdf') {
    exportHtml.value = false
    window.open(`${store.server.url_backend}/web/${file_report.value}` + '?' + paramsGet)
  }else if(values.tipe?.toLowerCase() == 'excel'){
      // exportHtml.value = false
    // window.open(`${store.server.url_backend}/public/presensi_absensi/exportPresensi?${paramsGet}`)

      exportHtml.value = false

      let endpoint = ''

      switch (values.tipe_report) {
        case 'Laporan Absensi Karyawan Detail':
          endpoint = 'exportPresensi'
          break
        case 'Laporan Rekap Absensi':
          endpoint = 'exportPresensiRekap'
          break
        case 'Laporan Realisasi Lembur':
          endpoint = 'exportRealisasiLembur'
          break
        case 'Laporan Pelanggaran Absensi':
          endpoint = 'exportPelanggaranPresensi'
          break
        default:
          endpoint = 'exportPresensi' // Fallback ke detail jika tidak cocok
      }

      window.open(`${store.server.url_backend}/public/presensi_absensi/${endpoint}?${paramsGet}`)
  } else {

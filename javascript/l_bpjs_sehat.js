...
// ------------------------------ PERSIAPAN
onBeforeMount(() => {
  document.title = 'Laporan BPJS Kesehatan'
})
...
const onGenerate = async () => {
  ...
  try {
    ...
    const url = `${store.server.url_backend}/public/t_perhitungan_gaji/exportBpjsKesehatan?${queryStr}`

    if (values.tipe?.toLowerCase() === 'html') {
      const res = await fetch(url)
      if (!res.ok) {
        const errJson = await res.json().catch(() => null)
        throw new Error(errJson?.error || 'Gagal memuat pratinjau tabel Laporan BPJS Kesehatan')
      }
      ...
      if (!res.ok) {
        const errJson = await res.json().catch(() => null)
        throw new Error(errJson?.error || 'Gagal mengunduh file Excel Laporan BPJS Kesehatan')
      }
      const blob = await res.blob()
      const downloadUrl = window.URL.createObjectURL(blob)
      const a = document.createElement('a')
      a.href = downloadUrl
      a.download = `Laporan_BPJS_Kesehatan_${values.periode}.xlsx`

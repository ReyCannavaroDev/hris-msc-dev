...
const landing = reactive({
    ...
    },
    {
      ...
      async click(row) {
        swal.fire({
          ...
          if (res.isConfirmed) {
            try {
              const dataURL = `${store.server.url_backend}/operation/t_lembur/post`
              isRequesting.value = true
              const res = await fetch(dataURL, {
                method: 'POST',
                headers: {
  ...
  api: {
    ...
    params: {
      ...
      where: `m_kary.m_dir_id=${store.user.data.m_dir_id} ${!store.user.data?.is_superadmin ? ('AND this.m_kary_id=' + store.user.data?.m_kary_id ?? 0) : ''}`
    },
    onsuccess(response) {

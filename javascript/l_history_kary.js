...
const values = reactive({
  tipe: 'HTML',
  kategori: 'Semua', // Pilihan: 'Semua', 'Mutasi & Kontrak', 'Riwayat Gaji', 'Surat & Disiplin'
  tipe_periode: 'Semua', // Pilihan: 'Semua', 'Bulan', 'Rentang Tanggal'
...
// ------------------------------ PERSIAPAN
onBeforeMount(() => {
  document.title = 'Laporan Riwayat (History) Karyawan'
})
...
const onGenerate = async () => {
  ...
  // Validasi periode jika tidak 'Semua'
  ...
  const params = {
    ...
    kategori: values.kategori ?? 'Semua',
  ...
  } else if (values.tipe_periode === 'Rentang Tanggal') {
  ...
  if (values.tipe.toLowerCase() === 'excel') {
    ...
    try {
      const url = `${store.server.url_backend}/public/m_kary/exportHistoryKaryawan?${queryStr}`
      const res = await fetch(url)
      if (!res.ok) throw new Error('Gagal mengunduh file Excel Laporan History Karyawan')
      const blob = await res.blob()
      const downloadUrl = window.URL.createObjectURL(blob)
      const a = document.createElement('a')
      a.href = downloadUrl
      let labelPeriod = 'Semua_Periode'
      if (values.tipe_periode === 'Bulan') {
        labelPeriod = values.periode
      } else if (values.tipe_periode === 'Rentang Tanggal') {
        labelPeriod = `${formatDateToYmd(values.date_start)}_sd_${formatDateToYmd(values.date_end)}`
      }
      a.download = `Laporan_History_Karyawan_${labelPeriod}.xlsx`
    ...
    try {
      const url = `${store.server.url_backend}/operation/m_kary/laporan_history_karyawan?${queryStr}`
      const res = await fetch(url, {
        headers: {
      ...
      if (!res.ok) throw new Error('Gagal menarik data pratinjau riwayat karyawan')
      const responseJson = await res.json()
      dataHistory.value = responseJson.data || []

  ...
  if (isRead) {
    try {
      ...
      if (route.query.is_approval) {
        dataURLAprv = `${store.server.url_backend}/operation/t_final_gaji/detail?id=${route.params.id}`
        ...
        isApproved.value = resultTrxJson?.data?.status_final === 'APPROVED'
        ...
        // Format periode_awal dan periode_akhir
        // const [tahunAwal, bulanAwal] = initialValues.periode_awal?.split('-') || []
        // const [tahunAkhir, bulanAkhir] = initialValues.periode_akhir?.split('-') || []
        // initialValues.periode_awal = `${tahunAwal}-${bulanAwal}`
        // initialValues.periode_akhir = `${tahunAkhir}-${bulanAkhir}`

        // Urutkan detail dan tambahkan nama karyawan
        ...
        isApproved.value = resultJson?.data?.status_final === 'APPROVED'
      }
    } catch (err) {
    ...
    },
    {
      ...
      async click(row) {
        swal.fire({
          ...
          if (res.isConfirmed) {
            try {
              const dataURL = `${store.server.url_backend}/operation/t_final_gaji/send_approval`
              isRequesting.value = true
              const res = await fetch(dataURL, {
                method: 'POST',
                headers: {

    ...
    <template #header>
      <div class="flex gap-x-2">
        <RouterLink :to="$route.path + '/create?' + Date.parse(new Date())"
          class="bg-green-500 text-white hover:bg-green-600 font-semibold rounded-[4px] py-1 px-[10px] flex items-center gap-1">
          Tambah
          <icon fa="plus" />
        </RouterLink>
        ...
        <button
      @click="downloadExcel"
      class="bg-yellow-500 text-white hover:bg-yellow-600 font-semibold rounded-[4px] py-1 px-[10px] flex items-center gap-1"
    >
      <icon fa="download" />
      Download
    </button>
  ...
  <div class="flex gap-x-4 px-2">
    <div class="flex flex-col border rounded shadow-sm px-6 py-6 <md:w-full w-full bg-white">
      ...
      <div class="flex items-center mb-2 border-b pb-4">
        ...
        <div v-show="!is_to_upload">
          <h2 v-if="!is_approval" class="mx-4 font-sans text-xl flex justify-left font-bold">
            {{actionText==='Edit'?'Ubah':actionText}} Cuti / Ijin
          </h2>
      ...
      <div class="grid <md:grid-cols-1 grid-cols-2 gap-2">
        ...
        <div>
          ...
          <FieldX :bind="{ readonly: true }" class="w-full !mt-0" :value="values.status"
            :errorText="formErrors.status?'failed':''" @input="v=>values.status=v" :hints="formErrors.status"
            :check="false" label="" placeholder="Tuliskan Status" />
        </div>
        <!-- <div>
        ...
        <div>
          ...
          <FieldX :bind="{ readonly: !actionText,disabled: !actionText }" type="date" class="w-full py-2 !mt-0"
            :value="values.date_from" label="" placeholder="Pilih Tanggal" :errorText="formErrors.date_from?'failed':''"
            @input="onDateChange" :hints="formErrors.date_from" :check="false" />
        </div>
        <div v-show="values.tipe_string?.toLowerCase() !== 'p24'">
          ...
          <FieldX :bind="{ readonly: !actionText,disabled: !actionText }" type="date" class="w-full py-2 !mt-0"
            :value="values.date_to" label="" placeholder="Pilih Tanggal" :errorText="formErrors.date_to?'failed':''"
        ...
        <div v-if="values.tipe_cuti_id === 15434">
          ...
          <FieldX :bind="{ readonly: !actionText,disabled: !actionText }" type="time" class="w-full py-2 !mt-0"
            :value="values.time_from" label="" placeholder="Masukan Waktu" :errorText="formErrors.time_from?'failed':''"
        ...
        <div v-if="values.tipe_cuti_id === 15434">
          ...
          <FieldX :bind="{ readonly: !actionText,disabled: !actionText }" type="time" class="w-full py-2 !mt-0"
            :value="values.time_to" label="" placeholder="Masukan Waktu" :errorText="formErrors.time_to?'failed':''"
        ...
        <div v-show="values.status === 'APPROVED'">
          <label class="col-span-12 font-semibold">Catatan Approval<label class="text-red-500 space-x-0 pl-0">*</label></label>
          <FieldSelect class="w-full py-2 !mt-0" :bind="{ readonly: !actionText,disabled: !actionText }" :value="values.approval_note" @input="v => values.approval_note = v"
            :errorText="formErrors.approval_note ? 'failed' : ''" :hints="formErrors.approval_note" valueField="key"
            displayField="key" :options="[
    { id: 100, key: 'Approve 100%' },
    { id: 75,  key: 'Approve 75%' },
    { id: 50,  key: 'Approve 50%' },
    { id: 50,  key: 'LATE/POTONG' }
  ]" placeholder="" fa-icon="" :check="false" />
        </div>
        <div v-show="values.status === 'REJECTED'">
          <label class="col-span-12 font-semibold">Catatan Approval<label class="text-red-500 space-x-0 pl-0">*</label></label>
         <FieldX  class="w-full py-2 !mt-0" :bind="{ readonly: true }" 
           :value="values.approval_note" :errorText="formErrors.approval_note?'failed':''"
           @input="v=>values.approval_note=v" :hints="formErrors.approval_note" 
           placeholder="" fa-icon="" :check="false"
         />
         
        </div>
        <div v-show="values.status === 'REVISED'">
          <label class="col-span-12 font-semibold">Catatan Approval<label class="text-red-500 space-x-0 pl-0">*</label></label>
         <FieldX  class="w-full py-2 !mt-0" :bind="{ readonly: true }" 
           :value="values.approval_note" :errorText="formErrors.approval_note?'failed':''"
           @input="v=>values.approval_note=v" :hints="formErrors.approval_note" 
        ...
        <!-- <div v-show="!actionText">
          ...
          <FieldX :bind="{  readonly: true }" class="w-full py-2 !mt-0" :value="values.interval"
            :errorText="formErrors.interval?'failed':''" @input="v=>values.interval=v" :hints="formErrors.interval"
            :check="false" label="" placeholder="Tuliskan Status" />
        </div> -->

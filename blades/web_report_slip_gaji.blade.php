@php
  ...
  if ($req->m_kary_id) {
    $m_kary_ids = [ $req->m_kary_id ];
  } else {
      $m_kary_ids = t_final_gaji_det::where('t_final_gaji_id', $req->f_id)
          ->pluck('m_kary_id')
          ->toArray();
  }
...
@foreach($m_kary_ids as $m_kary_id)
@php
  $raw = \DB::select("
    select f.periode_awal, f.nomor nomor_gaji, f.periode_akhir, d.total_gaji, f.desc,k.nama_lengkap, k.kode, k.nik,kd.nama dir, kdi.nama divisi, kde.nama dept, r.* from t_final_gaji_det_rincian r
  ...
  <tr>
    <td style="width: 50%; float: left;">
      <table style="border-collapse: collapse; width: 100%; font-size: 10px;">
        <tr>
          <td style="width: 25%; font-size: 10pt">ID Karyawan</td>
          <td style="font-size: 10pt">: {{ $raw[0]->kode }} </td>
        </tr>
        <tr>
        ...
        <tr>
          <td style="font-size: 10pt">Jabatan</td>
...
<table width="700px" style="font-size:10px; border: none;" cellpadding="1" id="lastTable">
  ...
  <tfoot style="font-weight:semibold;">
      <tr style="border: none;">
        <td style="font-weight: bold; line-height: 20px;text-align:center; background-color: #c8daf8;">Total Gaji</td>
        <td style="font-weight: bold; line-height: 20px;text-align:rignt; background-color: #c8daf8;">{{ formatRupiah($raw[0]->total_gaji) }}</td>
      </tr>
  </tfoot>
</table>
@endif

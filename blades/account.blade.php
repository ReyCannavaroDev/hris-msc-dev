    ...
    <div class="flex flex-col border rounded shadow-sm px-6 py-6 <md:w-full w-full bg-white">
      ...
      <div class="grid grid-cols-2 gap-2">
        ...
        <div class="flex flex-row justify-end space-x-[20px] mt-[10px]">
          <button 
              :disabled="((values.updated_num ?? 0) >= 2 && values.updated_year == new Date().getFullYear())"
              @click="redirectKary" 
              v-if="values.m_kary_id" 
              :class="[
                  ((values.updated_num ?? 0) >= 2 && values.updated_year == new Date().getFullYear())
                  ? 'bg-gray-400 hover:bg-gray-400 cursor-not-allowed' 
                  : 'bg-blue-500 hover:bg-blue-400 cursor-pointer'
              ]"
              class="text-white text-[10pt] font-semibold px-[36.5px] py-[10px] rounded-[6px]">
              <Icon fa="triangle-exclamation"/>
              {{ 
                  ((values.updated_num ?? 0) < 2 || values.updated_year != new Date().getFullYear())
                  ? 'Lengkapi Data Diri Anda' 
                  : 'Maksimal mengubah Dua Kali'
              }}
          </button>

...
{    
    ...
    public function custom_outstanding()
    {
        ...
        $is_manager = m_role::whereHas('m_role_access', function($q) use ($user_id){
        ...
        if($is_manager){
            $filteredData = $data->getCollection()->filter(function($item) use ($m_dir_id) {
                return $item['m_dir_id'] == $m_dir_id;
            })->values(); 

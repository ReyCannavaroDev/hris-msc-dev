...
class m_kary extends \App\Models\BasicModels\m_kary
{
    ...
    // {
    //     return $this->belongsTo('App\Models\BasicModels\t_cuti', 'id', 'm_kary_id');
    // }
    public function t_cuti() {
        return $this->hasMany(t_cuti::class, 'm_kary_id', 'id');
    }
    ...
    public function transformRowData(array $row)
    {
        ...
        /**
        * DETAIL REQUEST (extra data only when needed)
        */
        if (request('detail')) {
            $object['jadwal_kerja'] = \DB::table('t_jadwal_kerja as t')
        ...
        /**
        * SINGLE FETCH untuk kartu & kontrak — ambil SEKALI SAJA
        */
        $detKartu = \DB::table('m_kary_det_kartu')
            ->where('m_kary_id', $karyId)
            ->select('ktp_no')
            ->first();

        $detKontrak = m_kary_det_kontrak::where('m_karyawan_id', $karyId)
            ->orderBy('id') // atau tgl_awal, sesuai logika data
        ...
        /**
        * PERHITUNGAN SELISIH WAKTU (tanpa query tambahan)
        */
        ...
        * ATTRIBUT TAMBAHAN
        */
        $object['is_atasan'] = m_kary::where('atasan_id', $karyId)->exists();
        $object['nomor_ktp'] = $detKartu->ktp_no ?? null;
        $object['tgl_awal_kontrak'] = $kontrakPertama->tgl_awal ?? null;
        $object['tgl_akhir_kontrak'] = $kontrakAktif->tgl_akhir ?? null;
    ...
    public function custom_delete_kary($req)
    {
        \DB::beginTransaction();

        try {
            $kary = m_kary::find($req->id);

            if ($kary) {
                m_kary_det_kontrak::where('m_karyawan_id', $kary->id)->delete();
                $kary->delete();
            } else {
                return $this->helper->customResponse('Data karyawan tidak ditemukan', 404);
            }

            \DB::commit();
            return $this->helper->customResponse('Data berhasil dihapus');
        } catch (\Exception $e) {
            \DB::rollBack();
            return $this->helper->responseCatch($e);
        }
    ...
    public function public_importKary($req)
    {
        try{
           if ($req->hasFile('csv')) {
            $file = $req->file('csv');
            ...
            if (!in_array($ext, ['csv'])) {
                return response()->json(['error' => 'File harus berformat CSV'], 422);
            }
            ...
            $header = array_map(function($key) {
                return preg_replace('/[\x{FEFF}]/u', '', $key);
            }, $header);
            unset($data[0]);
            ...
            foreach ($data as $row) {
                ...
                if ($validator->fails()) {
                    DB::rollBack();
                    return response()->json([
                        'error' => 'Data tidak valid di baris: ' . json_encode($rowData),
                        'detail' => $validator->errors(),
                    ], 422);
                }

                $status_kerja = m_general::where('group', 'TIPE KARYAWAN')->where('key', $rowData['STATUS_KERJA'])?->first()?->id ?? 0;
                // dd($rowData);

                $status_nikah = m_general::where('group', 'TANGGUNGAN')->where('value', $rowData['STATUS_KARYAWAN'])?->first()?->id ?? 0;
                $unit = m_dir::where('nama', $rowData['UNIT'])?->first()?->id ?? 0;
                //$jabatan = m_divisi::where('nama', $rowData['JABATAN'])?->first()?->id ?? 0;
                $jabatan = m_divisi::firstOrCreate(
                    ['nama' => $rowData['JABATAN']], 
                    [
                        'status' => true,
                        'desc'=> $rowData['JABATAN']
                    ]
                )->id;
                $kota = m_general::where('group', 'KOTA')->whereRaw("LOWER(TRIM(REPLACE(REPLACE(value, 'Kota ', ''), 'Kabupaten ', ''))) = ?", [strtolower($rowData['KOTA'])])->first() ?? null;
                $presensi_lokasi = presensi_lokasi::where('nama', $rowData['UNIT'])?->first()?->id ?? 0;

                $m_kary_data = [
                    'kode' => $rowData['ID'],
                    'nama_lengkap' => $rowData['NAMA'],
                    'nama_depan' => $rowData['NAMA'],
                    'nama_belakang' => $rowData['NAMA'],
                    'nama_panggilan' => $rowData['NAMA'],
                    'tgl_lahir' => $rowData['TGL_LAHIR'],
                    'periode_gaji_id' => 362,
                    'alamat_domisili' => $rowData['ALAMAT'],
                    'alamat_asli' => $rowData['ALAMAT'],
                    'no_tlp' => $rowData['TELP'],
                    'm_dir_id' => $unit,
                    'm_divisi_id' => $jabatan,
                    'tipe_karyawan_id' => $status_kerja,
                    'tanggungan_id' => $status_nikah,
                    'kota_id' => $kota?->id ?? 0,
                    'provinsi_id' => $kota?->key ?? 0,
                    'presensi_lokasi_default_id' => $presensi_lokasi,
                    't_jadwal_kerja_id' => 37, //hardcode sementara ke office
                    'tipe_jam_kerja_id' => 990 //hardcode sementara ke office
                ];

                $m_kary = m_kary::updateOrCreate(
                    ['kode' => $rowData['ID']], 
                    $m_kary_data               
                );

                m_kary_det_kartu::updateOrCreate(
                    ['m_kary_id' => $m_kary->id],
                    ['desc_file' => $rowData['NO_IJAZAH']]
                );

                m_kary_det_pemb::updateOrCreate(
                    ['m_kary_id' => $m_kary->id],
                    [
                        'no_rek' => $rowData['NO_REK'] ?? 0,
                        'periode_gaji_id' => m_general::where('group', 'PERIODE GAJI')->where('value', 'BULANAN')->first()?->id ?? 0,
                    ]
                );
                // dd($rowData['TGL_AKHIR']);
                m_kary_det_kontrak::updateOrCreate(
                    ['m_karyawan_id' => $m_kary->id],
                    [
                        'm_divisi_id' => $jabatan,
                        'm_dir_id' => $unit,
                        'tipe_karyawan_id' => $status_kerja,
                        'status' => true,
                        'tgl_awal' => $rowData['TGL_MASUK'],
                        'tgl_akhir' => $rowData['TGL_AKHIR'] != "" ? $rowData['TGL_AKHIR'] : null,
                    ]
                );
            ...
            DB::commit();
             return response()->json(['success' => 'Import berhasil'], 200);
        }
    ...
    public function public_exportKary()
    {
        try {
            ...
            // dd($m_dir_id);
            // Ambil data yang relevan dengan kolom yang diimport
            $data = m_kary::with(['m_divisi', 'm_dir', 'm_kary_det_kartu', 'm_kary_det_pemb', 'm_kary_det_kontrak', 'kota', 'm_standart_gaji'])
                ->when($m_dir_id, function ($query) use ($m_dir_id) {
                    $query->where('m_dir_id', $m_dir_id);
                })
               ...
               ->map(function ($kary) {
                    ...
                    $gaji = $kary->m_standart_gaji?->gaji_pokok ?? 0;
                    ...
                    return [
                        ...
                        'TAHUN' => (string) $durasi['total_tahun'],

                        // BARIS INI YANG SUDAH DIBENARKAN
                        'TGL_AKHIR' => $tglAkhir ? $format($tglAkhir) : '',

                        'NO_REK' => $kary->m_kary_det_pemb->first()?->no_rek ?? '',
                        'NO_IJAZAH' => $kary->m_kary_det_kartu->first()?->desc_file ?? '',
                        'TGL_LAHIR' => $format($kary->tgl_lahir),
                        'ALAMAT' => $kary->alamat_domisili ?? '',
                        'KOTA' => $kary->kota?->value ?? '',
                        'TELP' => $kary->no_tlp ?? '',
                    ];
                });
            ...
            $export = new class($data) implements FromCollection, WithHeadings {
                ...
                {
                    return [
                        'ID', 'NAMA', 'GAJI', 'JABATAN', 'UNIT', 'TGL_MASUK',
                        'STATUS_KARYAWAN', 'STATUS_KERJA', 'BULAN', 'TAHUN', 'TGL_AKHIR','NO_REK', 'NO_IJAZAH',
                        'TGL_LAHIR', 'ALAMAT', 'KOTA', 'TELP'
                    ];
                }
            };
    ...
    // public function public_importKaryGaji($req)
    // {
    //     DB::beginTransaction();
    ...
    //         if (count($data) < 2) {
    //             return response()->json(['error' => 'File CSV tidak memiliki data'], 422);
    //         }
    ...
    //                 DB::rollBack();
    //                 return response()->json([
    //                     'error' => "Jumlah kolom tidak sesuai di baris ke-" . ($rowIndex),
    //                 ], 422);
    //             }
    ...
    //                 DB::rollBack();
    //                 return response()->json([
    //                     'error' => 'Data tidak valid di baris: ' . ($rowIndex),
    //                     'detail' => $validator->errors(),
    //                 ], 422);
    //             }

    //             // Ambil angka dari teks GAJI
    //             $gaji = (int) preg_replace('/[^0-9]/', '', $rowData['GAJI 2025']);
    //             if ($gaji <= 0) continue;

    //             // Cek atau buat data standar gaji
    //             $standartGaji = m_standart_gaji::firstOrCreate(
    //                 ['gaji_pokok' => $gaji],
    //                 [
    //                     'kode' => $this->helper->generateNomor('KODE STANDART GAJI'),
    //                     'is_active' => true,
    //                     'desc' => $gaji . ' - GAJI IMPORT EXCEL',
    //                     'gaji_pokok_periode' => 'Bulanan',
    //                     'created_at' => Carbon::now(),
    //                     'updated_at' => Carbon::now(),
    //                 ]
    //             );

    //             if ($standartGaji->wasRecentlyCreated) {
    //                 m_standart_gaji_det::create([
    //                     'm_standart_gaji_id' => $standartGaji->id,
    //                     'komponen' => 'TERLAMBAT 15',
    //                     'faktor' => '-',
    //                     'nilai' => 5000,
    //                     'periode' => 'Harian',
    //                     'tipe_komponen' => 'NOMINAL',
    //                     'created_at' => Carbon::now(),
    //                     'updated_at' => Carbon::now(),
    //                 ]);
    ...
    //             // Update karyawan berdasarkan kode ID
    //             m_kary::where('kode', $rowData['ID'])
    //                 ->update([
    //                     'm_standart_gaji_id' => $standartGaji->id,
    //                     'updated_at' => Carbon::now(),
    //                 ]);
    ...
    //         return response()->json([
    //             'message' => 'Import selesai. Gaji unik berhasil disimpan dan karyawan dimapping.',
    //         ]);
    ...
    public function public_importKaryGaji($req)
    {
        ...
        try {
            ...
            // 2. Cek ekstensi (Mendukung CSV dan Excel)
            if (!in_array($ext, ['csv', 'xls', 'xlsx'])) {
                return response()->json(['error' => 'File harus berformat CSV, XLS, atau XLSX'], 422);
            }

            // 3. Load file menggunakan PhpSpreadsheet
            $spreadsheet = IOFactory::load($file->getRealPath());
            $sheet = $spreadsheet->getActiveSheet();
            $data = $sheet->toArray(); // Konversi sheet menjadi array PHP

            if (count($data) < 2) {
                return response()->json(['error' => 'File tidak memiliki data'], 422);
            }

            // 4. Ambil header dan bersihkan karakter aneh/BOM
            $header = array_map('trim', $data[0]);
            unset($data[0]);
            $header[0] = preg_replace('/[\x00-\x1F\x80-\xFF]/', '', $header[0]);
            ...
            // 5. Looping data
            foreach ($data as $rowIndex => $row) {
                // Lewati jika baris kosong (sering terjadi di file Excel)
                if (empty(array_filter($row))) continue;

                if (count($row) !== count($header)) {
                    DB::rollBack();
                    return response()->json([
                        'error' => "Jumlah kolom tidak sesuai di baris ke-" . ($rowIndex + 1),
                    ], 422);
                }
                ...
                // Validasi field (sesuai kode asli Anda)
                $validator = Validator::make($rowData, [
                    'ID' => 'required',
                    'NAMA' => 'required',
                    'GAJI' => 'required',
                ...
                if ($validator->fails()) {
                    DB::rollBack();
                    return response()->json([
                        'error' => 'Data tidak valid di baris: ' . ($rowIndex + 1),
                        'detail' => $validator->errors(),
                    ], 422);
                }

                // Ambil angka dari teks GAJI (menggunakan key GAJI 2025 sesuai kode asli)
                $gaji = (int) preg_replace('/[^0-9]/', '', $rowData['GAJI']);
                if ($gaji <= 0) continue;
                ...
                // Jika record baru dibuat, buatkan detail potongannya
                if ($standartGaji->wasRecentlyCreated) {
                    m_standart_gaji_det::create([
                        'm_standart_gaji_id' => $standartGaji->id,
                        'komponen' => 'TERLAMBAT 15',
    ...
    public function public_updateKontrak()
    {
        $this->updateKontrak();
    }
    ...
    public function updateKontrak()
    {
        ...
        $id_seleksi = m_general::where('group', 'TIPE KARYAWAN')->where('key', 'S')->first()?->id ??0;
        $id_training = m_general::where('group', 'TIPE KARYAWAN')->where('key', 'TR')->first()?->id ??0;
        $id_kontrak = m_general::where('group', 'TIPE KARYAWAN')->where('key', 'K')->first()?->id ??0;

        $seleksi = m_kary_det_kontrak::where('tipe_karyawan_id', $id_seleksi)
            ->where('status', true)
            ->whereDate('tgl_awal', '<=', $today->copy()->subDays(7))
            ->get();

        foreach ($seleksi as $item) {
            $item->update(['status' => false]);

            $tgl_awal_baru = \Carbon\Carbon::parse($item->tgl_akhir)->addDay();
            $tgl_akhir_baru = $tgl_awal_baru->copy()->addMonths(3);

            m_kary_det_kontrak::create([
                'nomor'           => $this->helper->generateNomor("KODE KONTRAK"),
                'm_karyawan_id'   => $item->m_karyawan_id,
                'm_divisi_id'     => $item->m_divisi_id,
                'm_dir_id'        => $item->m_dir_id,
                'tipe_karyawan_id'=> $id_training,
                'status'          => true,
                'tgl_awal'        => $tgl_awal_baru,
                'duration'        => 3,
                'tgl_akhir'       => $tgl_akhir_baru,
                'created_at'      => Carbon::now(),
                'updated_at'      => Carbon::now(),
            ]);

            m_kary::where('id', $item->m_karyawan_id)->update([
                'm_divisi_id'     => $item->m_divisi_id,
                'm_dir_id'        => $item->m_dir_id,
                'tipe_karyawan_id'=> $id_training,
            ]);

            \Log::info("Auto insert: Seleksi → Training untuk karyawan ID {$item->m_karyawan_id}");
        }

        // $training = m_kary_det_kontrak::where('tipe_karyawan_id', $id_training)
        //     ->where('status', true)
        ...
        //         'm_divisi_id'     => $item->m_divisi_id,
        //         'm_dir_id'        => $item->m_dir_id,
        //         'tipe_karyawan_id'=> $id_kontrak,
        //         'status'          => true,
        //         'tgl_awal'        => $today,
        //         'duration'        => 6,
        //         'tgl_akhir'       => $today->copy()->addMonths(6),
        //         'created_at'      => now(),
        //         'updated_at'      => now(),
        //     ]);

        //     \Log::info("Auto insert: Training → Kontrak untuk karyawan ID {$item->m_karyawan_id}");
        // }

    }

    public function custom_countEnd($req)
    {  
        if(isset($req->tgl_awal) && isset($req->duration)){
            if($req->duration == 7){
                $tgl_akhir = Carbon::parse($req->tgl_awal)->addDays(7);
            }else{
                $tgl_akhir = Carbon::parse($req->tgl_awal)->addMonths($req->duration);
            }
            return response()->json([
    ...
    {
        return $model->select('m_kary.id', 'm_kary.kode','m_kary.nama_lengkap', 'm_kary.nama_depan', 'tipe_karyawan.value as tipe_karyawan.value' ,'tipe_karyawan.value as tipe_karyawan_value', 'm_dir.nama as m_dir.nama', 'm_divisi.nama as m_divisi.nama', 'm_kary.is_active');
    }

    public function custom_getTipeKaryawan()
    {
        $result = m_general::where('group', 'TIPE KARYAWAN')->select('id', 'key', 'value')->get();
        return $result;
    }

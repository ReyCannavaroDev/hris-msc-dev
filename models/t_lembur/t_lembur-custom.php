...
class t_lembur extends \App\Models\BasicModels\t_lembur
{
    ...
    public function public_exportLembur()
    {
        try {
            $fileName = 'data_lembur_' . Carbon::now()->format('Ymd_His') . '.xlsx';

            // Ambil data lembur + relasi yang relevan
            $data = t_lembur::with([
                    'm_kary.m_divisi',
                    'm_dir',
                    'tipe_lembur' => function ($q) { $q->select('id', 'value'); },
                ...
                ->map(function ($lembur) {
                    return [
                        'NOMOR' => $lembur->nomor ?? '',
                        'NAMA_KARYAWAN' => $lembur->m_kary?->nama_lengkap ?? '',
                        'DIVISI' => $lembur->m_kary?->m_divisi?->nama ?? '',
                        'UNIT' => $lembur->m_dir?->nama ?? '',
                        'TANGGAL' => $lembur->tanggal ? Carbon::parse($lembur->tanggal)->format('Y-m-d') : '',
                        'JAM_MULAI' => $lembur->jam_mulai ?? '',
                        'JAM_SELESAI' => $lembur->jam_selesai ?? '',
                        'TIPE_LEMBUR' => $lembur->tipe_lembur?->value ?? '',
                        'ALASAN' => $lembur->alasan?->value ?? '',
                        'NO_DOC' => $lembur->no_doc ?? '',
                        'DOKUMEN' => $lembur->doc ?? '',
                        'KETERANGAN' => $lembur->keterangan ?? '',
                        'STATUS' => $lembur->status ?? '',
                        'DIBUAT_OLEH' => $lembur->creator?->name ?? '-',
                        'DIBUAT_PADA' => $lembur->created_at ? $lembur->created_at->format('Y-m-d H:i:s') : '',
                    ];
                });
